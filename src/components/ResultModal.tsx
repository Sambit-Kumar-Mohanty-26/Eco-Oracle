...
import { Satellite, CheckCircle, XCircle, ExternalLink, X, Flame, Wind, Droplets, Leaf, AlertOctagon, Eye, AlertTriangle } from "lucide-react";
...
export default function ResultModal({ data, onClose }: ResultModalProps) {
  ...
  const getImageUrl = () => {
      ...
      return `${API_BASE}/temp/rgb_old.png?t=${Date.now()}`; // Past Photo
  };
  ...
  return (
    ...
    >
      ...
      <div className="h-56 w-full bg-gray-900 relative group">
         ...
         <img src={getImageUrl()} alt="Satellite Analysis" className="w-full h-full object-cover transition-opacity duration-500" />
         
         {!isGuardianMode && (
             <div className="absolute top-4 left-4 z-20 flex gap-2">
                <button onClick={() => setViewMode('ANALYSIS')} className={`text-[9px] font-bold px-3 py-1 rounded-full backdrop-blur-md border transition-all ${viewMode === 'ANALYSIS' ? 'bg-emerald-500 text-black border-emerald-500' : 'bg-black/50 text-white border-white/20'}`}>
                    AI ANALYSIS
                </button>
                <button onClick={() => setViewMode('VISUAL')} className={`text-[9px] font-bold px-3 py-1 rounded-full backdrop-blur-md border transition-all ${viewMode === 'VISUAL' ? 'bg-blue-500 text-black border-blue-500' : 'bg-black/50 text-white border-white/20'}`}>
                    REAL VISION
                </button>
             </div>
         )}
         ...
         {!isGuardianMode && viewMode === 'VISUAL' && (
             <div className="absolute bottom-12 right-4 z-20 flex bg-black/80 rounded-lg p-1 border border-white/20 backdrop-blur-md">
                <button onClick={() => setTimeTravel('PAST')} className={`px-3 py-1 rounded text-[9px] font-bold transition-all ${timeTravel === 'PAST' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}>1 YR AGO</button>
                <button onClick={() => setTimeTravel('NOW')} className={`px-3 py-1 rounded text-[9px] font-bold transition-all ${timeTravel === 'NOW' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}>TODAY</button>
             </div>
         )}
         ...
         {isGuardianMode && (
             <div className="absolute bottom-3 left-3 z-20">
                <span className="text-[9px] font-mono text-orange-400 bg-black/80 px-2 py-1 rounded border border-orange-500/50">NDMI MOISTURE LAYER</span>
             </div>
         )}
      </div>
            ...
            <>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-blue-900/20 p-3 rounded border border-blue-500/20">
                        ...
                        <div className="text-lg font-bold text-white">{weather.wind_speed} m/s</div>
                    </div>
                    <div className="bg-cyan-900/20 p-3 rounded border border-cyan-500/20">
                        ...
                        <div className="text-lg font-bold text-white">{riskData.dryness}%</div>
                    </div>
                </div>
            </>
        ) : (
            <>
               <div className="flex justify-between items-center">
                    <div>
                        ...
                        <div className="text-5xl font-black text-white">{aiData.biomass_score}%</div>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-zinc-900 p-3 rounded border border-white/5 relative overflow-hidden group">
                        <Leaf className="absolute top-2 right-2 text-emerald-500 opacity-20" size={32} />
                        <p className="text-[9px] text-gray-500 uppercase tracking-widest">Est. Carbon</p>
                        <p className="text-lg font-mono font-bold text-emerald-400">
                           {aiData.carbon_tonnes?.toLocaleString()} <span className="text-xs text-gray-600">t</span>
                        </p>
                    </div>
